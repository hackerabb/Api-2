...
# مفتاحك الصحيح والمجاني جاهز للعمل
...
@app.route('/')
def home():
    return jsonify({"status": "running", "message": "Nvn Live API is active. Use /api/live to get matches."})
...
def get_live_bets():
    try:
        ...
        sport_key = "soccer"  # جلب عام لكل مباريات كرة القدم لضمان وجود بيانات دائماً
        ...
        if isinstance(json_data, list):
            for game in json_data:
                ...
                if not team1 or not team2 or not bookmakers:
                    continue
        ...
        return jsonify({"status": "success", "count": 0, "results": [], "error": str(e)})
